...
class TableColumn(object):
    def __init__(
        ...
        self.start_with: Optional[int] = start_with
        self.increment_by: Optional[int] = increment_by
        self.last_created_identity: Optional[int] = None
...
def get_columns(ddl_statement: str) -> List[TableColumn]:
    ...
    for field in fields:
        ...
        if field and ("GENERATED BY DEFAULT AS IDENTITY" not in field):
            has_identity_value = False

        if not has_default_value and not has_identity_value:
        ...
        if field and has_identity_value:
            field = field.replace("GENERATED BY DEFAULT AS IDENTITY", "")
            field = field.replace("(", "")
            field = field.replace(")", "")
            field = field.replace("START WITH", "")
            field = field.strip()
            index = field.find(" ")

            start_with = int(field[:index])
            field = field[index:]
            field = field.replace("INCREMENT BY", "")
            field = field.strip()

            index = field.find(" ")

            if index != -1:
                increment_by = int(field[:index])
            else:
                increment_by = int(field)

        if field and has_default_value:
        ...
        _columns.append(
            TableColumn(
                column_name, column_type, default_value, start_with, increment_by
            )
        )
    return _columns
...
def put_row_on_table(table: Table, data: RowParam):
    ...
    for column in table["_columns"]:
        ...
        if value is None and column.start_with:
            column.last_created_identity = (
                column.start_with
                if column.last_created_identity is None
                else column.last_created_identity + column.increment_by
            )
            new_data[column.column_name] = column.last_created_identity
            continue
        ...
        if column.column_type in ["boolean"] and new_data[column.column_name] == "true":
    ...
    for filter in filters:
        ...
        for column in columns:
            if column.column_name == filter.column:
                if column.column_type in ["string"]:
                    filter.value = filter.value.replace("'", "")
                if column.column_type in ["number", "long", "float"]:
                    filter.value = float(filter.value)
                if column.column_type in ["integer"]:
                    filter.value = int(filter.value)
                if column.column_type in ["boolean"] and filter.value == "true":
                    filter.value = True
                if column.column_type in ["boolean"] and filter.value == "false":
                    filter.value = False
                if column.column_type in ["json"]:
